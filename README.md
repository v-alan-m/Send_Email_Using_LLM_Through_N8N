...
- Click on `Tool` + icon under `AI Agent` 
  ...
  - Click on `Create Google account` > Click opt left project dropdown > Click on `New Project`
    ...
    - Top left click on `hamburger menu` > Click on `API and Services` > Click on `OAuth consent screen`
    - Click on `Create`
      - Type App name e.g: `n8n_email_sender`
      - Type support email `email_address`
      - Type developer email `email_address`
      - Click on `Save and continue` > On next page click on `Save and continue`
      - Test users > CLick on `Add User` > Type `email address` > Click on `Add` > Click on `Save and continue`
      - Left column, click `Credentials` > Click on `Create credentials` > Click on `OAuth Client ID`
      - Select `Web Application` > Type name: `n8n_email_sender`
      - n8n copy URL `OAuth Redirect URL` > Google Cloud, Authorized redirect URIs, click on `Add URI` > Paste `OAuth Redirect URL` > Click on `Create`
